...
export function expandAgentPresetPlaceholders(
  ...
  return text.replace(PLACEHOLDER_PATTERN, (match, name: AgentPresetPlaceholderName) => expanders[name]());
}

/**
 * 由预设文本构造 ProposalAgent 系统提示词组合器（digest → 最终提示词）。
 * names 在任务提交时快照（对话切换守卫保证任务内 chat 不变）。
 */
export function composePresetSystemPrompt(
  presetText: string,
  names: AgentPromptNames,
): ProposalSystemPromptComposer {
  return (digest: MemorySpaceTableDigest) => expandAgentPresetPlaceholders(presetText, names, digest);
}
