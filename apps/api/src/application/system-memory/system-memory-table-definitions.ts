 ...
 * 绝对时间：以具体年月日开头（如「2025年5月14日 傍晚至23:10」「2025年5月15日」），
 * 日期后可跟任意时段/范围描述；无具体日期时才用「第 N 天·时段」稳定相对坐标。 */
const STORY_TIME_PATTERN =
  "^(?:20\\d{2}年\\d{1,2}月\\d{1,2}日.*|第\\s*[0-9一二两三四五六七八九十]+\\s*天[·、]?.+)$";
const STORY_TIME_MESSAGE =
  "时间坐标二选一：有明确日期时填具体年月日开头（如：2025年5月14日 傍晚至23:10、2025年5月15日 清晨）；否则填「第 N 天·时段」（如：第一天清晨），天数随剧情推进、跨入新的一天才 +1";
