...
- **手动验收（未执行，需用户在真实 ST 上做）**：`STE_ST_EXTENSION_DIR`/`STE_ST_INSTALL` 指向真实 ST 后跑 dev，刷新页面确认扩展管理器出现 STE Memory、控制台出现 `[STE Memory] v0.1.0 已加载` 日志（README 有步骤）。
...
- [x] 手动验收：ST 加载扩展成功，控制台有插件初始化日志（构建侧就绪，真机验收待用户配置 ST 目录后执行）
